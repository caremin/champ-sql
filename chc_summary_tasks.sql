CREATE OR REPLACE VIEW public.chc_summary_tasks
AS SELECT task_overview.chc_uuid,
   ...
   FROM ( SELECT tasks.doc ->> '_id'::text AS task_uuid,
            ...
            person.doc ->> '_id'::text AS hhmem_uuid,
           ...
           FROM raw_tasks tasks
             ...
             LEFT JOIN raw_contacts chc ON (chc.doc ->> '_id'::text) = (community.doc #>> '{contact,_id}'::text[])) task_overview(task_uuid, chc_uuid, hhmem_uuid, comm_uuid, hhmem_uuid_1, task_state, task_title, task_duedate, task_startdate, task_enddate, task_deleted, task_resolved, task_state_grp, task_completed_date, task_overdue_grp)
  GROUP BY task_overview.chc_uuid, task_overview.task_title, task_overview.task_overdue_grp
  ORDER BY task_overview.chc_uuid, task_overview.task_title, task_overview.task_overdue_grp;