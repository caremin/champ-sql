CREATE OR REPLACE VIEW public.reportview_malnutrition_with_issues_followup
AS SELECT reports.doc ->> '_id'::text AS report_uuid,
    to_timestamp((((reports.doc ->> 'reported_date'::text)::bigint) / 1000)::double precision) AS report_date,
    reports.doc ->> 'form'::text AS which_report,
    reports.doc #>> '{fields,patient_uuid}'::text[] AS hhmem_uuid,
    reports.doc #>> '{contact,_id}'::text[] AS chc_uuid,
    reports.doc #>> '{fields,patient_id}'::text[] AS patient_id,
    reports.doc #>> '{fields,patient_name}'::text[] AS patient_name,
    reports.doc #>> '{fields,patient_gender}'::text[] AS patient_gender,
    reports.doc #>> '{fields,inputs,contact,date_of_birth}'::text[] AS birthdate,
    reports.doc #>> '{fields,patient_age_in_months}'::text[] AS patient_age_inmonths,
    reports.doc #>> '{fields,patient_age_in_years}'::text[] AS patient_age_inyears,
    reports.doc #>> '{fields,inputs,contact,parent,parent,contact,name}'::text[] AS parent_name,
    reports.doc #>> '{fields,inputs,contact,parent,parent,contact,phone}'::text[] AS parent_contact,
    reports.doc #>> '{fields,followup_details_1,continue}'::text[] AS continue_followup,
    reports.doc #>> '{fields,followup_details_2,visit_rhu}'::text[] AS visit_rhu,
    reports.doc #>> '{fields,followup_details_3,diagnosis}'::text[] AS final_diagnosis,
    reports.doc #>> '{fields,followup_details_3,visit_date}'::text[] AS date_visitation,
    reports.doc #>> '{fields,followup_details_3,intervention}'::text[] AS interventions,
    reports.doc #>> '{fields,followup_details_3,issues_resolved}'::text[] AS resolved_issues,
    reports.doc #>> '{fields,followup_details_3,why_not_resolved}'::text[] AS issues_description
   FROM raw_reports reports
  WHERE (reports.doc ->> 'form'::text) = 'malnutrition_with_issues_followup'::text
  ORDER BY (to_timestamp((((reports.doc ->> 'reported_date'::text)::bigint) / 1000)::double precision)) DESC;