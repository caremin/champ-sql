SELECT reports.doc ->> '_id'::text AS report_uuid,
    ...
    reports.doc #>> '{fields,g_child_ask,continue_child}'::text[] AS child_continue,
    ...
    reports.doc #>> '{fields,g_child,g_malnutrition,chc_has_scale}'::text[] AS chc_has_scale,
    reports.doc #>> '{fields,g_child,g_malnutrition,child_weight}'::text[] AS child_weight,
    CASE 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_scale,equipment_reason_scale}'::text[])='equip_never_received' THEN 'Never received equipment'::TEXT 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_scale,equipment_reason_scale}'::text[])='equip_lost' THEN 'Equipment lost'::TEXT 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_scale,equipment_reason_scale}'::text[])='equip_forgot' THEN 'Equipment forgotten today'::TEXT 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_scale,equipment_reason_scale}'::text[])='equip_battery' THEN 'No or dead battery for weighing scale'::TEXT 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_scale,equipment_reason_scale}'::text[])='equip_damaged' THEN 'Damaged or non-functional equipment'::TEXT 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_scale,equipment_reason_scale}'::text[])='equip_oth' THEN 'Other'::TEXT 
    	ELSE reports.doc #>> '{fields,g_child,g_malnutrition,g_scale,equipment_reason_scale}'::text[]
    END  AS reason_for_no_scale,
    reports.doc #>> '{fields,g_child,g_malnutrition,g_scale,equipment_reason_scale_other}'::text[] AS other_reason_for_no_scale,
    reports.doc #>> '{fields,g_child,g_malnutrition,g_child_weight,chc_has_stadiometer}'::text[] AS chc_has_stadiometer,
    reports.doc #>> '{fields,g_child,g_malnutrition,child_height}'::text[] AS child_height,
    CASE 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_no_stadiometer,equipment_reason_stadiometer}'::TEXT[])='equip_never_received' THEN 'Never received equipment'::TEXT 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_no_stadiometer,equipment_reason_stadiometer}'::TEXT[])='equip_lost' THEN 'Equipment lost'::TEXT 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_no_stadiometer,equipment_reason_stadiometer}'::TEXT[])='equip_forgot' THEN 'Equipment forgotten today'::TEXT 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_no_stadiometer,equipment_reason_stadiometer}'::TEXT[])='equip_damaged' THEN 'Damaged or non-functional equipment'::TEXT 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_no_stadiometer,equipment_reason_stadiometer}'::TEXT[])='equip_oth' THEN 'Other'::TEXT 
    	ELSE reports.doc #>> '{fields,g_child,g_malnutrition,g_no_stadiometer,equipment_reason_stadiometer}'::TEXT[]
    END AS reason_for_no_stadiometer,
    reports.doc #>> '{fields,g_child_g_malnutrition,g_no_stadiometer,equipment_reason_stadiometer_other}'::text[] AS other_reason_for_no_stadiometer,
    reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfa_score}'::text[] AS wfa_zscore,
    CASE 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfa_classification}'::TEXT[])='unknown' THEN 'Unknown'::TEXT
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfa_classification}'::TEXT[])='normal' THEN 'Normal'::TEXT
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfa_classification}'::TEXT[])='underweight' THEN 'Underweight'::TEXT
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfa_classification}'::TEXT[])='severely_underweight' THEN 'Severely Underweight'::Text
    	ELSE reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfa_classification}'::TEXT[]
    END AS wfa_classification,
    reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfh_score}'::text[] AS wfh_zscore,
    CASE 
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfh_classification}'::TEXT[])='unknown' THEN 'Unknown'::TEXT
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfh_classification}'::TEXT[])='normal' THEN 'Normal'::TEXT
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfh_classification}'::TEXT[])='underweight' THEN 'Underweight'::TEXT
    	WHEN (reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfh_classification}'::TEXT[])='severely_underweight' THEN 'Severely Underweight'::Text
    	ELSE reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,wfh_classification}'::TEXT[]
    END AS wfh_classification,
    reports.doc #>> '{fields,g_child,g_malnutrition,g_recalculate_zscore,suspected_malnourished}'::text[] AS suspected_malnourished,
    ...
    reports.doc #>> '{fields,g_tb_ask,continue_tb}'::text[] AS tb_continue,
    ...
    reports.doc #>> '{fields,g_pregnancy,pregnancy_details_1,pregnancy_age}'::text[] AS preg_datails_totalmths,
    ...
    reports.doc #>> '{fields,g_pregnancy,history,previous_children}'::text[] AS preg_history_prevnum,
    reports.doc #>> '{fields,g_pregnancy,history,birth_place_before}'::text[] AS preg_history_prevbirthplace,
    reports.doc #>> '{fields,g_pregnancy,check_up,visits_required}'::text[] AS preg_checkup_prenat_visitsreq,
    reports.doc #>> '{fields,g_pregnancy,check_up,num_prenatal_visit}'::text[] AS preg_checkup_prenat_visitsattend,
